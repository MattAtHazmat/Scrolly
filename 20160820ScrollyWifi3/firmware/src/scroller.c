...
bool SCROLLER_Initialize ( void )
{
    ...
    scrollerData.display.info.timeDate = true;
    return true;
}
...
void SCROLLER_Tasks ( void )
{
    switch ( scrollerData.state )
    {
        ...
        case SCROLLER_STATE_OPEN_SPI:
        {
            ...
            {
                scrollerData.display.SPIHandle = 
                    DRV_SPI_Open(SCROLLER_SPI_DRIVER, 
                                 DRV_IO_INTENT_EXCLUSIVE | DRV_IO_INTENT_WRITE | DRV_IO_INTENT_NONBLOCKING);     
        ...
        case SCROLLER_STATE_SEND_STRIP:
        {
            ...
            {
                scrollerData.counters.displaySent++;
        ...
        case SCROLLER_STATE_DETERMINE_START:
        {
            ...
            if(scrollerData.display.stringOffsetColumn<(-scrollerData.display.stringColumns))
            {
                ...
                scrollerData.display.stringOffsetColumn = scrollerData.display.info.columns;
                /* switch, if there is data to switch to */
                if(scrollerData.display.info.message&&scrollerData.display.info.displayMessage)
                {
                    DisplayNewMessage(&scrollerData.display,scrollerData.display.message);   
                    /* switch to display the other next time */
                    if(scrollerData.display.info.timeDate)
                    {
                        scrollerData.display.info.displayMessage = false;
                    }
                }
                else
                {
                    scrollerData.display.info.timeDate = GetCurrentTimeString(scrollerData.display.time);
                    /* get rid of the leading zero if it is there */
                    if(scrollerData.display.time[0] == '0')
                    {
                        scrollerData.display.time[0] = ' ';                        
                    }
                    DisplayNewMessage(&scrollerData.display,scrollerData.display.time);
                    if(scrollerData.display.info.message)
                    {
                        scrollerData.display.info.displayMessage = true;
                    }
                }
            }
            scrollerData.state = SCROLLER_STATE_POPULATE_BUFFER;
...
void ScrollerEnableTime(bool enable)
{
    EnableTime(&scrollerData.display,enable);
}
/*******************************************************************************
 End of File
 */
