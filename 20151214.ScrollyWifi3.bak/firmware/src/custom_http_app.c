...
extern SCROLLER_DATA scrollerData;
...
HTTP_IO_RESULT TCPIP_HTTP_PostExecute(HTTP_CONN_HANDLE connHandle)
{
    ...
    {
        return HTTPPostScroller(connHandle);
    }
#if defined(SYS_OUT_ENABLE)
    if(!memcmp(filename, "forms.htm", 9))
...
void TCPIP_HTTP_Print_message(HTTP_CONN_HANDLE connHandle)
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    //TCPIP_NET_HANDLE netH = TCPIP_TCP_SocketNetGet(sktHTTP);
    TCPIP_TCP_StringPut(sktHTTP, scrollerData.display.characterString);
}
...
{   
    ...
    while(TCPIP_HTTP_CurrentConnectionByteCountGet(connHandle))
    {
        ...
        /* Parse the value that was read                                      */
        if(!strcmp((char *)httpDataBuff, (const char *)"msg"))
        {   
            if(strlen((char *)(httpDataBuff+6)) > DISPLAY_STRING_LENGTH)
            {
                bConfigFailure = true;
                break;
            }
            if(!DisplayNewMessage(&scrollerData.display,httpDataBuff + 6))
            {
                bConfigFailure = true;
                break;
            }
        }
        else if(!strcmp((char *)httpDataBuff, (const char *)"fgc"))
        {
            uint8_t foregroundColor;
            if(!TCPIP_Helper_StringToColorCode((char *)(httpDataBuff+6), &foregroundColor))
            {
                bConfigFailure = true;
                break;
            }
            if(!DisplaySetForegroundColor(&scrollerData.display,foregroundColor))
            {
                bConfigFailure = true;
                break;
            }
        }
        else if(!strcmp((char *)httpDataBuff, (const char *)"bgc"))
        {
            uint8_t backgroundColor;
            if(!TCPIP_Helper_StringToColorCode((char *)(httpDataBuff+6), &backgroundColor))
            {
                bConfigFailure = true;
                break;
            }
            if(!DisplaySetBackgroundColor(&scrollerData.display,backgroundColor))
            {
                bConfigFailure = true;
                break;
            }
...
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    if(scrollerData.display.foregroundColor == RED_COLOR_MAP_INDEX)
    {
        TCPIP_TCP_StringPut(sktHTTP, CHECKED_STRING);
    }
}
/******************************************************************************/
...
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    if(scrollerData.display.foregroundColor == GREEN_COLOR_MAP_INDEX)
    {
        TCPIP_TCP_StringPut(sktHTTP, CHECKED_STRING);
    }
}
...
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    if(scrollerData.display.foregroundColor == BLUE_COLOR_MAP_INDEX)
    {
        TCPIP_TCP_StringPut(sktHTTP, CHECKED_STRING);
    }
}
...
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    if(scrollerData.display.foregroundColor == BLACK_COLOR_MAP_INDEX)
    {
        TCPIP_TCP_StringPut(sktHTTP, CHECKED_STRING);
    }
}
...
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    if(scrollerData.display.foregroundColor == WHITE_COLOR_MAP_INDEX)
    {
        TCPIP_TCP_StringPut(sktHTTP, CHECKED_STRING);
    }
}
...
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    if(scrollerData.display.foregroundColor == YELLOW_COLOR_MAP_INDEX)
    {
        TCPIP_TCP_StringPut(sktHTTP, CHECKED_STRING);
    }
}
...
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    if(scrollerData.display.foregroundColor == CYAN_COLOR_MAP_INDEX)
    {
        TCPIP_TCP_StringPut(sktHTTP, CHECKED_STRING);
    }
}
...
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    if(scrollerData.display.foregroundColor == VIOLET_COLOR_MAP_INDEX)
    {
        TCPIP_TCP_StringPut(sktHTTP, CHECKED_STRING);
    }
}
...
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    if(scrollerData.display.backgroundColor == RED_COLOR_MAP_INDEX)
    {
        TCPIP_TCP_StringPut(sktHTTP, CHECKED_STRING);
    }
}
...
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    if(scrollerData.display.backgroundColor == GREEN_COLOR_MAP_INDEX)
    {
        TCPIP_TCP_StringPut(sktHTTP, CHECKED_STRING);
    }
}
...
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    if(scrollerData.display.backgroundColor == BLUE_COLOR_MAP_INDEX)
    {
        TCPIP_TCP_StringPut(sktHTTP, CHECKED_STRING);
    }
}
...
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    if(scrollerData.display.backgroundColor == BLACK_COLOR_MAP_INDEX)
    {
        TCPIP_TCP_StringPut(sktHTTP, CHECKED_STRING);
    }
}
...
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    if(scrollerData.display.backgroundColor == WHITE_COLOR_MAP_INDEX)
    {
        TCPIP_TCP_StringPut(sktHTTP, CHECKED_STRING);
    }
}
...
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    if(scrollerData.display.backgroundColor == YELLOW_COLOR_MAP_INDEX)
    {
        TCPIP_TCP_StringPut(sktHTTP, CHECKED_STRING);
    }
}
...
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    if(scrollerData.display.backgroundColor == CYAN_COLOR_MAP_INDEX)
    {
        TCPIP_TCP_StringPut(sktHTTP, CHECKED_STRING);
    }
}
...
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    if(scrollerData.display.backgroundColor == VIOLET_COLOR_MAP_INDEX)
    {
        TCPIP_TCP_StringPut(sktHTTP, CHECKED_STRING);
    }
}
